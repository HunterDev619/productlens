...
export type DateRangePickerProps = {
  ...
  onUpdateAction?: (values: { range: DateRange; rangeCompare?: DateRange }) => void;
...
type PresetButtonProps = { label: string; isSelected: boolean; onClick: () => void };
const PresetButton = ({ label, isSelected, onClick }: PresetButtonProps): JSX.Element => (
  <Button
...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  // Selected preset is derived from current range
  ...
  // removed duplicate local getPresetRange (top-level version is used)
  ...
  // selectedPreset is derived via useMemo, no effect needed

  // removed duplicate nested PresetButton (top-level version is used)

  // Helper function to check if two date ranges are equal
  const areRangesEqual = (a?: DateRange, b?: DateRange): boolean => {
    if (!a || !b) {
      return a === b;
    } // If either is undefined, return true if both are undefined
    return (
      ...
      <PopoverTrigger asChild>
        <Button size="lg" variant="outline">
          ...
          <div className="-mr-2 scale-125 pl-1 opacity-60">
            {isOpen ? (<CaretUp width={24} />) : (<CaretDown width={24} />)}
          </div>
        </Button>
      </PopoverTrigger>
          ...
          <Button
            onClick={() => {
              ...
              if (
                ...
                onUpdateAction?.({ range, rangeCompare });
              }
            }}
          >
            Update
