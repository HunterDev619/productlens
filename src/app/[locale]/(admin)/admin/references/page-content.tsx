...
import { useAdminListCategories, useAdminListCategoryOptions } from '@/services/admin/categories/list';
import { useAdminUpdateCategory } from '@/services/admin/categories/update';
import { useAdminCreateReference } from '@/services/admin/references/create';
import { useAdminDeleteReference } from '@/services/admin/references/delete';
import { useAdminListReferences } from '@/services/admin/references/list';
import { useAdminUpdateReference } from '@/services/admin/references/update';
...
type PageMode = 'references' | 'categories';

export default function ReferencesPageContent() {
  const [mode, setMode] = useState<PageMode>('references');

  // References hooks
  const { data: referencesData, isLoading: isReferencesLoading } = useAdminListReferences();
  const { createReference, isPending: isCreating } = useAdminCreateReference();
  const { mutateAsync: updateReference, isPending: isUpdating } = useAdminUpdateReference();
  const { mutateAsync: deleteReference, isPending: isDeleting } = useAdminDeleteReference();
  const { options: categoryOptions, isLoading: isCategoriesLoading } = useAdminListCategoryOptions();

  // Categories hooks
  const { data: categoriesData, isLoading: isCategoriesListLoading } = useAdminListCategories();
  const { createCategory, isPending: isCreatingCategory } = useAdminCreateCategory();
  const { mutateAsync: updateCategory, isPending: isUpdatingCategory } = useAdminUpdateCategory();
  const { mutateAsync: deleteCategory, isPending: isDeletingCategory } = useAdminDeleteCategory();
  ...
  // Form states for references
  const [formData, setFormData] = useState<CreateReferencePayload>({
    categoryId: '',
    name: '',
    description: '',
    url: null,
  ...
  const handleEdit = async () => {
    if (!selectedReferenceId || !formData.name.trim()) {
      return;
    }
    try {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteReference(selectedReferenceId);
  ...
  const handleEditCategory = async () => {
    if (!selectedCategoryId || !categoryFormData.title.trim()) {
      return;
    }
    try {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => openEditCategoryDialog(row.original)}
            >
              <Pencil size={16} />
            </Button>
            <Button
              variant="outline"
              size="sm"
              onClick={() => openDeleteCategoryDialog(row.original)}
  ...
  const referencesTable = useReactTable({
    data: referencesData?.data || [],
  ...
  const isLoading = mode === 'references' ? isReferencesLoading : isCategoriesListLoading;
      ...
      <div className="flex min-h-screen items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-lg font-medium text-gray-700">
            Loading
            {' '}
            {mode === 'references' ? 'References' : 'Categories'}
            ...
          </p>
        </div>
      </div>
  ...
  const selectedReference = referencesData?.data?.find(ref => ref.id === selectedReferenceId);
  ...
  return (
    <div className="min-h-screen p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {mode === 'references'
          ? (
              <StatsCards references={referencesData?.data || []} />
            )
          : (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
        ...
        {mode === 'references'
          ? (
              <ReferencesTable table={referencesTable} columns={referenceColumns} />
            )
          : (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {categoriesTable.getRowModel().rows.length > 0
                        ...
                        : (
                            <tr>
                              <td colSpan={categoryColumns.length} className="px-6 py-12 text-center">
                  ...
                  <div className="flex items-center justify-between border-t border-gray-200 bg-gray-50 px-6 py-4">
                    <div className="text-sm text-gray-600">
                      ...
                      <span className="font-medium">
                        {categoriesTable.getState().pagination.pageIndex * categoriesTable.getState().pagination.pageSize + 1}
                      </span>
                      {' '}
                      to
                      {' '}
                      <span className="font-medium">
                        {Math.min(
                          (categoriesTable.getState().pagination.pageIndex + 1) * categoriesTable.getState().pagination.pageSize,
                          categoriesTable.getFilteredRowModel().rows.length,
                        )}
                      </span>
                      {' '}
                      of
                      {' '}
                      <span className="font-medium">{categoriesTable.getFilteredRowModel().rows.length}</span>
                      {' '}
                      results
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        {Array.from({ length: categoriesTable.getPageCount() }, (_, i) => i).map((pageNum) => {
                          const isCurrentPage = pageNum === categoriesTable.getState().pagination.pageIndex;
                          const showPage
                            = pageNum === 0
                              || pageNum === categoriesTable.getPageCount() - 1
                              || Math.abs(pageNum - categoriesTable.getState().pagination.pageIndex) <= 1;

                          if (!showPage) {
                            if (
                              pageNum === 1
                              && categoriesTable.getState().pagination.pageIndex > 2
                            ) {
                              return <span key={pageNum} className="px-2 text-gray-400">...</span>;
                            }
                            if (
                              pageNum === categoriesTable.getPageCount() - 2
                              && categoriesTable.getState().pagination.pageIndex < categoriesTable.getPageCount() - 3
                            ) {
                              return <span key={pageNum} className="px-2 text-gray-400">...</span>;
                            }
                            return null;
                          ...
                          return (
                            <button
                              key={pageNum}
                              type="button"
                              onClick={() => categoriesTable.setPageIndex(pageNum)}
                              className={cn(
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    disabled={isCreatingCategory || !categoryFormData.title.trim()}
                    className="bg-emerald-600 hover:bg-emerald-700"
                  >
                    {isCreatingCategory
                      ? (
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    disabled={isUpdatingCategory || !categoryFormData.title.trim()}
                    className="bg-emerald-600 hover:bg-emerald-700"
                  >
                    {isUpdatingCategory
                      ? (
