import { GlobeHemisphereEast, MapTrifold } from '@phosphor-icons/react';
...
export default function SupplyChainTraceabilityAndTransportation({ lcaAnalysis_id }: SupplyChainTraceabilityAndTransportationProps) {
  const { data: lcaAnalysisData, isLoading, error } = useGetLcaAnalysisDetail(lcaAnalysis_id);
  const mapRef = useRef<HTMLDivElement | null>(null);
  const mapInstanceRef = useRef<any>(null);
  const distributionRoutes = lcaAnalysisData?.data?.otherAnalysis?.geographic_analysis?.supply_chain_mapping?.distribution_routes || [];
  const rawMaterials = lcaAnalysisData?.data?.otherAnalysis?.geographic_analysis?.supply_chain_mapping?.raw_materials || [];
  const manufacturingHubs = lcaAnalysisData?.data?.otherAnalysis?.geographic_analysis?.supply_chain_mapping?.manufacturing_hubs || [];
  ...
  useEffect(() => {
    ...
    if (error) {
      console.error('Failed to load supply chain & geography.', error);
      return;
    }

    console.warn('Failed to load supply chain & geography: missing lcaAnalysis data.');
  }, [hasError, error]);
  ...
  // Lazy load Leaflet from CDN and draw routes
  useEffect(() => {
    ...
    ensureLeaflet().then((L) => {
      ...
      const getUniqueLatLng = (lat: number, lng: number): { lat: number; lng: number } => {
        ...
        // Duplicate found, offset by count * 0.1 degrees (alternating lat/lng)
        if (count % 2 === 1) {
      ...
      // Normalize various coordinate shapes to LatLngs
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 sm:space-y-6">
        <div className="h-5 w-48 animate-pulse rounded bg-gray-200 sm:h-6 sm:w-64" />
        <div className="h-64 w-full animate-pulse rounded bg-gray-100 sm:h-96" />
      </div>
    );
  }
  ...
  const otherAnalysis = lcaData.otherAnalysis;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center gap-2">
        <GlobeHemisphereEast size={16} className="h-4 w-4 text-sky-600 sm:h-[18px] sm:w-[18px]" />
        ...
        <div className="space-y-6">
          ...
          {supplyChainMapping.tier_breakdown && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
              <h5 className="mb-3 text-base font-semibold text-gray-900 sm:mb-4 sm:text-lg">Supplier Tier Breakdown</h5>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-4 md:grid-cols-4">
                {supplyChainMapping.tier_breakdown.tier_1_suppliers !== undefined && (
                  <div className="rounded-lg bg-blue-50 p-3 text-center sm:p-4">
                    ...
                    <p className="mt-1 text-xs text-gray-600 sm:text-sm">Tier 1 Suppliers</p>
                  </div>
                )}
                {supplyChainMapping.tier_breakdown.tier_2_suppliers !== undefined && (
                  <div className="rounded-lg bg-green-50 p-3 text-center sm:p-4">
                    ...
                    <p className="mt-1 text-xs text-gray-600 sm:text-sm">Tier 2 Suppliers</p>
                  </div>
                )}
                {supplyChainMapping.tier_breakdown.tier_3_suppliers !== undefined && (
                  <div className="rounded-lg bg-yellow-50 p-3 text-center sm:p-4">
                    <p className="text-2xl font-bold text-yellow-700 sm:text-3xl">
                      {supplyChainMapping.tier_breakdown.tier_3_suppliers}
                    </p>
                    <p className="mt-1 text-xs text-gray-600 sm:text-sm">Tier 3 Suppliers</p>
                  </div>
                )}
          ...
          {supplyChainMapping.transportation_network && supplyChainMapping.transportation_network.length > 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
              ...
              <div className="mb-3 flex flex-col gap-3 sm:mb-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2">
                  <Truck size={18} className="h-5 w-5 text-gray-600 sm:h-6 sm:w-6" />
                  <h5 className="text-lg font-semibold text-gray-900 sm:text-xl">Transportation Distance & Emissions</h5>
                </div>
                <div className="text-left sm:text-right">
                  <p className="text-sm text-gray-600 sm:text-base">Total Distance</p>
      ...
      {geographicAnalysis && (
        <div className="space-y-6">
          ...
          {geoSupplyChainMapping && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
              <h5 className="mb-3 text-lg font-semibold text-gray-900 sm:mb-4 sm:text-xl">Supply Chain Materials</h5>
          ...
          {regionalImpactIntensity && (
            <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-6">
              <h5 className="mb-3 text-lg font-semibold text-gray-900 sm:mb-4 sm:text-xl">Regional Impact Intensity</h5>
